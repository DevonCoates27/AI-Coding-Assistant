...
void JobSystem::createJob(std::string jobType, unsigned long channels, std::string name){
    if(jobType == "compileJob"){
        Job* newJob = new CompileJob(channels, 1, name);
        // QueueJob(newJob);
        makeJob(newJob);
        std::cout << "CREATED COMPILE JOB " << name << std::endl;
    }
    else if(jobType == "renderJob"){
        Job* newJob = new RenderJob(channels, 2, name);
        makeJob(newJob);
        std::cout << "CREATED RENDER JOB " << name << std::endl;
    }
    else if(jobType == "subtractJob"){
        Job* newJob = new SubtractJob(channels, 3, name);
        makeJob(newJob);
        std::cout << "CREATED SUBTRACT JOB " << name << std::endl;
    }
    else if(jobType == "customJob"){
        Job* newJob = new CustomJob(channels, 4, name);
        makeJob(newJob);
        std::cout << "CREATED CUSTOM JOB " << name << std::endl;
    }
    else if(jobType == "promptJob"){
        Job* newJob = new PromptJob(channels, 5, name);
        makeJob(newJob);
        std::cout << "CREATED PROMPT JOB " << name << std::endl;
    }
}

void JobSystem::destroyJob(int jobID){
    for(int i = 0; i < m_jobsQueued.size(); i++){
        if(m_jobsQueued.at(i)->GetUniqueID() == jobID){
            m_jobsQueued.at(i)->~Job();
        }
    }
    for(int i = 0; i < m_jobsRunning.size(); i++){
        if(m_jobsRunning.at(i)->GetUniqueID() == jobID){
            m_jobsRunning.at(i)->~Job();
        }
    }
    for(int i = 0; i < m_jobsCompleted.size(); i++){
        if(m_jobsCompleted.at(i)->GetUniqueID() == jobID){
            m_jobsCompleted.at(i)->~Job();
        }
    }
}
...
void JobSystem::queueNamedJob(std::string name){
    for(int i = 0; i < m_jobsUnqueued.size(); i++){
        if(m_jobsUnqueued.at(i)->m_name == name){
            QueueJob(m_jobsUnqueued.at(i));
        }
    }
}

void JobSystem::makeJob(Job* job){
    m_jobsUnqueued.push_back(job);
}